import { Instagram, Facebook, Github, Store, ShoppingBag } from "lucide-react";
    ...
    <footer className="fixed bottom-0 left-0 right-0 z-40 py-2 bg-background/90 border-t border-border/30">
      <div className="container mx-auto px-3">
        <div className="flex items-center justify-between">
          {/* Right side - Store, Cart, WhatsApp */}
          <div className="flex items-center gap-2">
            <a 
              href="#cookies"
              className="w-8 h-8 rounded-full bg-primary/20 border border-primary/30 flex items-center justify-center hover:bg-primary hover:border-transparent transition-all duration-300 hover:scale-110 group"
            >
              <Store className="w-4 h-4 text-primary group-hover:text-white transition-colors" />
            </a>
            <a 
              href="#"
              onClick={(e) => {
